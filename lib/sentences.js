...
// find if there's duplicates in this area in the best time complexity
function areDuplicates(arr){

  let previousElements = Set();

  for (let i = 0; i < arr.length; i++){
    if (previousElements.has(arr[i])){
      return true;
    }
    previousElements.add(arr[i]);
  }
  return false;
}
...
// list
// map / hashtable / dictionary / javascript object -> key value pair that is fast to access
// set / like the keys in a map are a keyset 
const sentences = {
